...
class ZhEnMapping(models.Model):
    ...
    Lưu trữ từ khóa tiếng Anh đã được làm sạch (Sanitized Keywords) 
    từ dữ liệu HSK 1-9, phục vụ cho quá trình xây dựng Prompt 
    gửi đến mô hình AI tạo ảnh (Generative AI).
    
    Ưu tiên tuyệt đối trường en_translation thay vì cột translation_en 
    thuộc bảng gốc ZhWord — vì dữ liệu gốc thường chứa noise data 
    (dấu phẩy, cụm từ giải nghĩa bổ sung) làm sai lệch vector ngữ cảnh AI.
    """
    ...
    en_translation = models.CharField(
        max_length=255,
        help_text="Từ khóa tiếng Anh sạch dùng làm Prompt cho AI (sanitized keyword)"
    )
    ...
    def __str__(self):
        return f"{self.zh_word} → {self.en_translation}"
